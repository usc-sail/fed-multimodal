    ...
    if Path.exists(output_data_path.joinpath(f'partition.pkl')) == False:
        ...
        for client_id in client_id_list:
            ...
            label_df = pd.read_csv(
                Path(args.raw_data_dir).joinpath(
            ...
            # if there is not watch data should just skip
            if Path.exists(Path(args.raw_data_dir).joinpath('ExtraSensory/watch_acc', client_id)) == False:
                logging.info(f'Skip processing for {client_id}')
                continue
            ...
            for index in tqdm(list(label_df.index), ncols=100, miniters=100):
                ...
                # we make sure it is not multi-label classification
                if np.nansum(row_df) == 1:
                    acc_file_path = Path(args.raw_data_dir).joinpath(
                        'ExtraSensory/raw_acc', 
                        client_id, 
                        str(index)+'.m_raw_acc.dat'
                    )
                    watch_acc_file_path = Path(args.raw_data_dir).joinpath(
                        'ExtraSensory/watch_acc', 
                        client_id, 
                        str(index)+'.m_watch_acc.dat'
                    )
