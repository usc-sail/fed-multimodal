...
if __name__ == '__main__':
    ...
    for client_id in tqdm(dm.client_ids):
        # load audio features
        audio_dict = [data for data in dm.load_audio_feat(client_id=client_id) if len(data) == 9]
        # load video features
        video_dict = [data for data in dm.load_video_feat(client_id=client_id) if len(data) == 9]
        
        dm.get_label_dist(
            video_dict, client_id
        )
        ...
        dataloader_dict[client_id] = dm.set_dataloader(
            ...
            default_feat_shape_a=np.array([150, constants.feature_len_dict["whisper_tiny"]]),
    ...
    for fold_idx in range(1, 6):
        ...
        global_model = MMActionClassifier(
            ...
            audio_input_dim=constants.feature_len_dict["whisper_tiny"], 
        ...
        for epoch in range(int(args.num_epochs)):
            ...
            if epoch % args.test_frequency == 0 or epoch == int(args.num_epochs)-1:
                with torch.no_grad():
                    ...
                    server.log_classification_result(
                        data_split='dev', metric='auc'
                    )

                    # 4. We just use dev
                    server.result_dict[epoch]['test'] = server.result
                    server.log_classification_result(
                        data_split='test', metric='auc'
                    )
                
                logging.info('---------------------------------------------------------')
                server.log_epoch_result(metric='auc')
            logging.info('---------------------------------------------------------')
    ...
    for metric in ['auc', 'acc', 'top5_acc']:
        result_list = list()
        for key in save_result_dict:
            if metric not in save_result_dict[key]: continue
            result_list.append(save_result_dict[key]["auc"])
        save_result_dict['average']["auc"] = np.nanmean(result_list)
